...
module.exports = (function() {
    ...
    mailRoutes.post('/send', function (req, res) {
        ...
        if (!mail && !mobile) {
            return res.send({
                success: false,
                message: 'Please enter at last one way to contact you or call me directly on  0208 675 9754.'
            });
       }
       ...
       if (mail&&!emailRegex.test(mail)) {
           return res.send({
               success: false,
               message: 'Please enter a valid address or send an emial directly to georgie@calvocoressi.com.'
           });
      }
       const mobRegex=/^[+]?(\d+([- ])?)*$/;
      ...
      console.log(location)
       if (!location) {
         ...
         //secure: true for 465, false for other ports
            ...
            text+= req.body.mail? ' Email them back by clicking on the "Reply" button':'';
            ...
            text+= (req.body.mobile && req.body.mail)? ' or call '+req.body.mobile:'';
            text+='.';
            const mailOptions = {
              from: process.env.MAIL_USER,
              to: process.env.MAIL_USER,
              replyTo,
              subject: 'Consultaion request received.',
              text
            };
